...
export default function BookingModal({ isOpen, onClose, preselectedServiceId }: BookingModalProps) {
  ...
  // Real-time listener for the selected date to prevent double booking
  useEffect(() => {
    if (step !== 2) return;
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setAvailableSlots(prevSlots => {
        if (prevSlots.length === 0) return prevSlots; // Wait for initial load
      ...
      // If the user's currently selected time just got booked by someone else, deselect it
  ...
  const handleBook = async (e: React.FormEvent) => {
    ...
    // Trecem INSTANT la pasul de succes
    ...
    try {
      ...
      // Salvează în Firebase și așteaptă confirmarea (în fundal)
      ...
      // Trigger Email & Calendar sync on backend (fire and forget to not block UI)
      fetch('/api/book', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          phone: formData.phone,
          email: formData.email,
          serviceName: service?.name || selectedService,
          date: format(selectedDate, 'yyyy-MM-dd'),
          time: selectedTime
        })
      }).catch(e => console.error("Backend sync error", e));

      // Wait for firebase save in background
      try {
        await Promise.race([
          addDocPromise,
        ...
        console.warn('Firebase save timeout/error, but email was triggered', fbError);
      }
    } catch (error) {
                  ...
                  <form onSubmit={handleBook} className="flex flex-col flex-1 min-h-0">
                    <div className="space-y-4 flex-1 overflow-y-auto pr-2 pb-2 custom-scrollbar">
                      ...
                      <div>
                        ...
                        <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full border border-sage-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-gold-400/50 focus:border-gold-400 text-sage-900 bg-white shadow-sm transition-all" placeholder="exemplu@email.com" />
                      </div>
